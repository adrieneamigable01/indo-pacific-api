...
class LoanModel extends Model
{
    ...
    public function getLoans($search = '', $where = [])
    {
     ...
     public function getLoanServerside( $search='',
        $where=[],
        $start=0,
        $length=10,
        $orderColumn='loan_id',
    ...
    )
    {
        ...
        $orderBy = $allowedColumns[$orderColumn] ?? 'l.loan_id';
        ...
        foreach ($loans as &$loan) {
            ...
            */
            ...
            foreach($schedules as &$schedule){
                $schedule['loan_product_id'] = (int)$loan['loan_product_id'];
                $paymentTotals = $this->db
                    ->table('loan_payments')
                    ->select("
                        COALESCE(SUM(principal_amount),0) as principal_paid,
    ...
    public function countLoans($where = [])
    {
        $builder = $this->db
            ->table('loans l')
            ->join(
    ...
    public function countFilteredLoans($search = '', $where = [])
    {
        $builder = $this->db
            ->table('loans l')
            ->join(
